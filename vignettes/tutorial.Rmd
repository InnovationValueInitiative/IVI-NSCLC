...
Two R packages are needed to simulate the model: the `iviNSCLC` and [hesim](https://innovationvalueinitiative.github.io/hesim/) packages. We will also use [ggplot2](https://ggplot2.tidyverse.org/) and [scales](https://scales.r-lib.org/) for plotting and the [data.table](https://github.com/Rdatatable/data.table/wiki) package to manipulate data and simulation output.
...
costs[, strategy_name := factor(strategy_id, levels = 1:length(txseqs), 
                                 labels = paste0("Sequence ", 1:length(txseqs)))]
...
We might also want to examine incremental costs relative to a reference treatment (say treatment sequence 1). 

```{r incr_costs, fig.width = 4}
# Incremental costs with Strategy 1 as the reference treatment
incr_costs <- ce_sim$costs[, costs_ref := costs[1], 
                           by = c("category", "dr", "sample")]
incr_costs[, icosts := costs - costs_ref]
...
incr_costs[, strategy_name := factor(strategy_id, levels = 1:length(txseqs), 
                                 labels = paste0("Sequence ", 1:length(txseqs)))]
