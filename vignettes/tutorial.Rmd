...
stprobs[, state_name := factor(state_id, levels = 1:nrow(states), 
                                 labels = states$state_name)]
stprobs[, strategy_name := factor(strategy_id, levels = 1:length(txseqs), 
...
curves[, strategy_name := factor(strategy_id, levels = 1:length(txseqs), 
                                 labels = paste0("Sequence ", 1:length(txseqs)))]
...
median_rows <- curves[, .I[which(prob_mean <= 0.5)[1]], 
                      by = c("strategy_id", "lab")]$V1
median_dt <- curves[median_rows, ]
median_dt[, strategy_name := factor(strategy_name, 
                                    levels = rev(unique(strategy_name)))]
...
#### QALYs
Mean QALYs and life-years can be simulated by treatment sequence and health state. We simulate values without a discount rate and using a 3 percent discount rate, and plot undiscounted life-years.
...
lys[, state_name := factor(state_id, levels = 1:nrow(states), 
                                 labels = states$state_name)]
lys[, strategy_name := factor(strategy_id, levels = rev(1:length(txseqs)), 
                                 labels = rev(paste0("Sequence ", 1:length(txseqs))))]
