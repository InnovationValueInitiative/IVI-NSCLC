...
```{r ae_probs_plot, fig.height = 4}
tidy_ae_probs <- tidy(ae_probs)
plot_data <- tidy_ae_probs[, .(prob_mean = mean(prob),
                               prob_lower = quantile(prob, .025),
                                prob_upper = quantile(prob, .975)),
                          by = c("strategy_id", "ae_name")]
plot_data[, ":=" (num_mean = 1000 * prob_mean,
                  num_lower = 1000 * prob_lower,
                  num_upper = 1000 * prob_upper)]
...
plot_data[, ae_name := ifelse(ae_name == "Elevated aspartate transaminase",
                              "Elevated \n aspartate \n transaminase",
                              ae_name)]
ggplot(plot_data, aes(x = rev(strategy_name), y = num_mean)) + 
  facet_wrap(~factor(ae_name), nrow = 3) +
  geom_bar(stat = "identity", fill = "#d9230f") +
  geom_errorbar(aes(ymin = num_lower,
                    ymax = num_upper), width = .2) +
  scale_fill_discrete(name = "") +
  scale_y_continuous(label = scales::comma) +
  xlab("") + ylab("Number of adverse events per 1,000 patients") + 
  coord_flip() 
```
