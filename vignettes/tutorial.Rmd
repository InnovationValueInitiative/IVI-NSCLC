...
Two R packages are needed to simulate the model: the `iviNSCLC` and [hesim](https://innovationvalueinitiative.github.io/hesim/) packages. We will also use [ggplot2](https://ggplot2.tidyverse.org/) and [scales](https://scales.r-lib.org/) for plotting. Note that the `iviNSCLC` loads the [data.table](https://github.com/Rdatatable/data.table/wiki) package, which we will use to manipulate data and simulation output.
...
```{r mean_costs, fig.width = 4}
# Simulate
econmod$sim_costs(dr = .03)
ce_sim <- econmod$summarize()

# Plot
costs <- ce_sim$costs[ , .(costs_mean = mean(costs),
