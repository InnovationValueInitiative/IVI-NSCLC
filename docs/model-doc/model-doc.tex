...
Relevant evidence to estimate treatment effects with the alternative TKIs and chemotherapy regimens in terms of transitions between the health states was obtained with a systematic literature review of RCTs evaluating the efficacy of relevant competing interventions for the treatment of adult patients with metastatic EGFR+ non-squamous NSCLC by line of treatment (i.e. 1L, 2L, and 2L+) in terms of OS and PFS. Details regarding eligibility criteria, included studies, extracted information regarding study and patient characteristics, results, and KM curves are provided in \autoref{app:km-curves}. 
...
It is important to note that the available PFS and OS data for the different interventions varied dramatically. At the time when the systematic literature review was performed, there was about 25 months of follow-up data available for osimertinib as 1L treatment, with median OS not yet reached; about 40 months of data for dacomitinib; 40-50 months for afatinib; and more than 50 months of follow-up data for gefitinib and erlotinib. This implies that treatment effect estimates for osimertinib and dacomitinib are much more uncertain at this stage than the other interventions and are likely to change when longer term data becomes available. See \autoref{fig:ARCHER-1050}-\autoref{fig:Yang 2014} for more detail.
...
Health state utility values (EQ-5d, HUI2, HUI-3, SF-6D) for the different health states were used in the model as well as disutility estimates associated with adverse events identified by focusing on review studies. Based on these review studies, the primary studies were identified when deemed relevant for the model and available. Furthermore, published mapping algorithms that allow a non-preference-based measure (generic or disease-specific measure) to be mapped onto a generic preference-based measure are of interest, as well as mapping algorithms between different generic preference-based health state utility values (e.g. between SF-6D and EQ5D) were searched for, in anticipation that there were no studies providing utility estimates that could directly be linked to PFS and OS related health states used in the model. Details regarding eligibility criteria defining the scope of the studies considered relevant are outlined in \autoref{tbl:picos-utilities}. 
...
Relevant evidence regarding resource use, productivity and cost estimates were identified by means of a review of published cost-of-illness studies, cost-effectiveness studies, and budget impact studies in NSCLC relevant for the US setting. Details regarding eligibility criteria defining the scope of the studies considered relevant are outlined in \autoref{tbl:picos-cost}. Studies with relevant data among EGFR+NSCLC  patients and treated with TKIs were use in the economic model, with a preference for more recent publications.
...
Despite the strengths of the systematic literature review to identify relevant evidence, some limitations are applicable that should be acknowledged. As the evidence base is continually growing, any trials published after the search date will not be captured. Furthermore, there is a risk of publication bias. Finally, the search and selection were restricted to studies published in English.

\section{Evidence base, efficacy and safety}
\subsection{Study identification and selection} \label{app:study-selection}
The identification and selection of relevant studies used to estimate treatment effects is summarized with \autoref{fig:slr-flow}.
...
\end{subfigure}
\begin{subfigure}{\textwidth}
\includegraphics[max size={\textwidth}{\textheight}]{figs/km-plots/.....png}
\end{subfigure}
\centering
\caption{AURA-3 and AURA-2, progression-free survival and overall survival with osimertinib (T790+)}\label{fig:osimertinib-2l}
\end{figure}
