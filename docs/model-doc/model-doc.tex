...
A fixed effects model is obtained by replacing $\delta_{1,ik} &\sim N(d_{1, t_{ik}} - d_{1, t_{i1}}, \sigma^2)$ with $lambda_{1,ik} &= d_{1, t_{ik}} - d_{1, t_{i1}}$. 
...
For each interval $m$, the proportions  $S_{ik}(u)$, $P_{ik}(u)$, and $D_{ik}(u)$ are related to the time-varying hazards $\ h_{ik}^{SP} (u) $,$\ h_{ik}^{SD} (u) $, $\ h_{ik}^{PD} (u)$ according to a set of differential equations \citep{jansen2013multivariate} expressed in  \autoref{eqn:diff_equations}. 
...
The prior distributions for the parameters of the model expressed with \autoref{eqn:multistate-nma} are displayed in \autoref{eqn:multistate-nma-priors}.
...
The prior distribution for the probabilities at the beginning of each interval, $u_{m}$, is presented in \autoref{eqn:multinomial-prior}. 
...
The residual deviance and the deviance information criterion (DIC) wereused to compare the goodness-of-fit of the competing NMA models \citep{dias2018network}. DIC provides a measure of model fit that penalizes model complexity. In general, a more complex model results in a better fit to the data, demonstrating a smaller residual deviance. The model with the better trade-off between fit and parsimony has a lower DIC. A difference in the DIC of about 10 points was considered meaningful. 
